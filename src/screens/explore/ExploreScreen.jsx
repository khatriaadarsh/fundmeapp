...
const CampaignCard = memo(({ item, onPress }) => (
  ...
  >
    ...
    <View style={ccSt.info}>
      <Text style={ccSt.title} numberOfLines={2}>
        {item.title}
      </Text>
      ...
      <View style={ccSt.meta}>
        ...
        <Text style={ccSt.user} numberOfLines={1}>
          {item.user}
        </Text>
        {item.verified && (
...
const ExploreScreen = ({ navigation }) => {
  ...
  const runSearch = useCallback((text, cat) => {
    ...
    if (cat !== 'all') {
      res = res.filter(c => c.category === cat);
    }
    if (q) {
      res = res.filter(
        c =>
...
const scSt = StyleSheet.create({
  ...
  searchRow: {
    ...
    backgroundColor: P.white,
    paddingHorizontal: sp(14),
    // ✅ FIXED: removed STATUSBAR_H — SafeAreaView handles it
    paddingTop: sp(10),
    paddingBottom: sp(10),
    gap: sp(10),
