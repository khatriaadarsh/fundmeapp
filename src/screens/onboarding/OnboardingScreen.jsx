...
// Fixes: the "next screen overlapping" bug — AboutScreen now renders
// inside a real <Modal>, so it's guaranteed to sit in its own native
// layer above everything (no more zIndex/stacking gamble, no partial
// coverage). Its entrance is also now an instant full-bleed background
// + staggered content fade, instead of a slide-up reveal — so there's
// never a frame where the onboarding screen shows through underneath it.
// handleStart still calls markOnboardingSeen() before navigating so the
// flag is persisted to AsyncStorage.
//
...
// ─── AboutScreen ────────────────────────────────────────────
// Renders inside a <Modal> from OnboardingScreen (see below), so it's
// guaranteed to occupy its own native layer above everything — no zIndex
// stacking assumptions, no partial coverage of the screen underneath.
//
// The gradient background now appears at full opacity the instant this
// mounts (no slide-up-from-bottom reveal), so there is never a frame
// where the onboarding screen shows through behind it — that reveal
// window was the actual cause of the "overlap" glitch. Only the INNER
// content (logo, title, features, tagline) still does a gentle staggered
// fade/slide-in, purely for polish, on top of an already-solid background.
const AboutScreen = memo(({ visible, onStart }) => {
  const contentFade = useRef(new Animated.Value(0)).current;
  const contentSlide = useRef(new Animated.Value(scale(30))).current;
    ...
    if (visible) {
      Animated.parallel([
        ...
        Animated.stagger(120, [
          Animated.parallel([
            Animated.timing(feat1, {
              toValue: 1,
              duration: 350,
              useNativeDriver: true,
            }),
            Animated.spring(featSlide1, {
              toValue: 0,
              tension: 60,
              friction: 8,
              useNativeDriver: true,
          ...
          Animated.parallel([
            Animated.timing(feat2, {
              toValue: 1,
              duration: 350,
              useNativeDriver: true,
            }),
            Animated.spring(featSlide2, {
              toValue: 0,
              tension: 60,
              friction: 8,
              useNativeDriver: true,
          ...
          Animated.parallel([
            Animated.timing(feat3, {
              toValue: 1,
              duration: 350,
              useNativeDriver: true,
            }),
            Animated.spring(featSlide3, {
              toValue: 0,
              tension: 60,
              friction: 8,
              useNativeDriver: true,
      ...
      contentSlide.setValue(scale(30));
      feat1.setValue(0);
      feat2.setValue(0);
      feat3.setValue(0);
  ...
  const features = [
    {
      icon: 'shield',
      text: 'CNIC Verified Creators',
      desc: 'Every fundraiser is identity verified',
      anim: feat1,
      slide: featSlide1,
    },
    {
      icon: 'zap',
      text: 'AI Fraud Protection',
      desc: 'Smart detection keeps donations safe',
      anim: feat2,
      slide: featSlide2,
    },
    {
      icon: 'heart',
      text: '0% Platform Fee',
      desc: '100% of your donation reaches those in need',
      anim: feat3,
      slide: featSlide3,
    },
  ];
  ...
  return (
    <View style={abSt.fullScreen}>
      ...
      >
        ...
        <Text style={abSt.subtitle}>
          Pakistan's Most Trusted Crowdfunding Platform
        </Text>
...
const OnboardingScreen = ({ navigation }) => {
  ...
  // Android hardware back button while the About modal is open should
  // close it and return to the onboarding cards, not exit the app.
  useEffect(() => {
    if (Platform.OS !== 'android' || !showAbout) return undefined;
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {/*
        AboutScreen renders inside a real Modal instead of as an absolutely-
        positioned sibling. A Modal is guaranteed to draw in its own native
        layer above everything on screen — including the status bar — on
        both platforms, which is what actually fixes the overlap: there's
        no zIndex/stacking-order assumption left to break on any device.
      */}
      <Modal
        visible={showAbout}
        animationType="none"
