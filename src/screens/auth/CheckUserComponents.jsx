...
const heroSt = StyleSheet.create({
  container: {
    width: '100%',
    height: sp(280), // Slightly taller to ensure overlap looks good
    borderBottomLeftRadius: sp(40),
    borderBottomRightRadius: sp(40),
    overflow: 'hidden',
    alignItems: 'center',
...
export const EmailInputField = memo(({
  value, onChangeText, onFocus, onBlur, isFocused, isValid, hasError, isLoading, errorMsg, helperMsg,
}) => {
  const borderColor = hasError ? '#EF4444' : isFocused ? '#00B4CC' : '#E5E7EB';
  ...
  return (
    <View style={inSt.wrapper}>
      ...
      <View style={[inSt.box, { borderColor }]}>
        <Icon name="mail" size={sp(18)} color={iconColor} style={inSt.leftIcon} />
        <TextInput
          style={inSt.field}
          value={value}
          ...
          editable={!isLoading}
        />
        {isValid && !hasError && <Icon name="check-circle" size={sp(18)} color="#22C55E" style={inSt.rightIcon} />}
      </View>
      {hasError && errorMsg ? (
      ...
      ) : helperMsg ? <Text style={inSt.helperTxt}>{helperMsg}</Text> : null}
    </View>
  );
});
...
const inSt = StyleSheet.create({
  ...
  field: { flex: 1, fontSize: sp(14), color: '#111827', paddingVertical: 0 },
...
const badgeSt = StyleSheet.create({
  row: { flexDirection: 'row', justifyContent: 'center', gap: sp(20), marginTop: sp(28), marginBottom: sp(16) },
...
const footSt = StyleSheet.create({
  wrap: { alignItems: 'center', paddingBottom: sp(24) },
  caption: { fontSize: sp(11), color: '#B0B7C3', marginBottom: sp(4) },
  links: { fontSize: sp(11), color: GRAD_START, fontWeight: '600', textDecorationLine: 'underline' },
});